    ...
    public function fileUpload(Request $request)
    {
        ...
        if(!empty($path) && array_key_exists($path, $this->valid_path)) {
            if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
                    ...
                    if(in_array($ext, $this->valid_ext)) {
                        ...
                        if($isDimension) {
                            if($file->move($path,$image_name)) {
                                ...
                                $max_sl_no = RzAshramGallery_provider::where('valid', 1)
                        ...
                        $first_image = RzAshramGallery_provider::where('valid', 1)
                               ->where('photo_gallery_id', $request->idParameter)
                               ->orderBy('sl_no') 
                               ->first(); 
                ...
                RzAshramPhotoGallery_provider::valid()->find($request->idParameter)->update([
