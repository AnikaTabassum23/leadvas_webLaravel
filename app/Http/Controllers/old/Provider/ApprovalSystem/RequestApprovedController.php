...
class RequestApprovedController extends Controller
{
    ...
    public function requestAprrovedStatusAction(Request $request)
    {
        $output = array();
        $input  = $request->all();
        $employeeAuthId = Auth::guard('provider')->user()->id;
        ...
        $service_req_info = ServiceRequestedChain_provider::valid()->where('service_req_id',$service_req_id)->get()->pluck('employee_id')->toArray();
        $last_employee_id = end($service_req_info);
        ...
        $plannerAction = ActionPlanners_provider::join('service_category','service_category.id','=','action_planners.service_category_id')
        ->join('service_request','service_request.service_category_id','=','service_category.id')
        ->select('action_planners.id','action_planners.employee_id as planner_id')
        ->where('service_request.id',$service_req_id)
        ->where('action_planners.contact_person_status',1)
        ...
        if ($validator->passes()) {
            ...
            //if authId = last_employee_id
            //update into planner table 
            if($last_employee_id==$employeeAuthId){
                ServiceRequest_provider::find($service_req_id)->update([
                    'final_approved_status'          => 1,
                    'final_approved_date'            => $current_date,
                    'planner_id'                     => $plannerAction->planner_id
                ]);
            }
