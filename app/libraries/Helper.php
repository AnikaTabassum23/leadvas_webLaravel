...
class Helper {
    ...
    public static function mailConfig($configFrom='', $user_id=0, $configId=0) {
        $mailConfig = '';
        $config = array();
        // if($configFrom=='provider') {
        //     if($configId>0) {
        //         $mailConfig = EnProviderMailConfiguration::where('valid', 1)->find($configId);
        //     } else if($user_id>0) {
        //         $mailConfig = EnProviderMailConfiguration::where('valid', 1)->where('assign_to', $user_id)->first();
        //     } else {
        //         $project_id = ((Auth::provider()->check())) ? Auth::provider()->get()->project_id : '';
        //         if(!empty($project_id)) {
        //             $mailConfig = EnProviderMailConfiguration::where('valid', 1)->where('assign_to', 0)->where('project_id', $project_id)->first();
        //         }
        //     }
        // } else if($configFrom=='corporate') {
        //     dd(12);
        //     if($configId>0) {
        //         $mailConfig = EnCorporateMailConfiguration::where('valid', 1)->find($configId);
        //     } else if($user_id>0) {
        //         $mailConfig = EnCorporateMailConfiguration::where('valid', 1)->where('assign_to', $user_id)->first();
        //     } else {
        //         $corporate_id = ((Auth::corporate()->check())) ? Auth::corporate()->get()->corporate_id : '';
        //         if(!empty($corporate_id)) {
        //             $mailConfig = EnCorporateMailConfiguration::where('valid', 1)->where('assign_to', 0)->where('corporate_id', $corporate_id)->first();
        //         }
        //     }
        // }

        // if(!empty($mailConfig)) {
        //     if($mailConfig->mail_protocal==1) {
        //         $config = array(
        //             'driver'        => 'sendmail',
        ...
        //         'port'          => 465,
        //         'from'          => [
        //         'address'       => 'no-reply@eduvess.com',
        //         'name'          => 'Eduvess'
        //         ],
        //         'encryption'    => 'starttls',
        //         'username'      => 'no-reply@eduvess.com',
        //         'password'      => '000NOREPLY000',
        //         'sendmail'      => '/usr/sbin/sendmail -bs',
        //         'pretend'       => false
        //    ];
        // }
    ...
    public static function getProductCode($table){
        $date = date("ym");
        $output = array();
        $master_table = DB::table($table)->where(DB::raw("(DATE_FORMAT(created_at,'%y%m'))"),$date)->get();
        if(count($master_table) > 0)
        {
            foreach ($master_table as $value)
            {
                $output[$value->de_product_code] = substr($value->de_product_code,0,4);
            }
        }
        $check = in_array($date, $output);
        if($check) {
            $filter = array_intersect($output, [$date]);
            $max = max(array_keys($filter));
            $get_counter =  substr($max,4,1);
            $get_counter = $get_counter+1;
            //$get_counter = str_pad($get_counter, 5, "0", STR_PAD_LEFT);
            $value =   $date.$get_counter;
        } else {
            $value = $date."1";
        }
        return $value;
