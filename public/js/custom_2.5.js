...
function multipleFileUpload(selector, postFix='') {
	...
	$(function() {
        ...
        $selector.after('<input type="file" class="filestyle" id="'+selector+'">');
        $selector.remove();
        $('#'+selector).filestyle({
            input: !0,
            icon: !0,
        ...
        $('#'+selector).change(function () {
            ...
            if (myfile == '') {
                ...
                if(!ext || (extArray && extExist==0)) {
                    ...
                    return;
                } else if(uploadfile[0].files[0].size>524288000) {
                    ...
                    formData.append('uploadfile'+postFix, $(this)[0].files[0]);
...
function assignNotifyPermit(id, permit, $notif_btn, $lst_btn, $dtl_btn) {
    if($notif_btn.length>0) { var $notif_parent=$notif_btn.parents('li').first(); customPreloader($notif_parent); }
    if($lst_btn.length>0) { var $lst_parent=$lst_btn.parents('tr').first(); customPreloader($lst_parent); }
    if($dtl_btn.length>0) { var $dtl_parent=$dtl_btn.parents('.panel').first(); customPreloader($dtl_parent); }
