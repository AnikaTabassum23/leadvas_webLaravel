...
<meta property="og:image" content="{{url('public/uploads/gallery_coverphoto/'.$albam->cover_photo)}}" />
...
@section('content')
  <section>
     {{--<div class="back-repository" >
            <a class="previous round" href="{{ route('photography') }}">< </a>
            
        </div> --}}

          <div id="ex1" class="modal shareSocialModal">
            <a href="" data-url="{{ route('gallery',['id' => $albam->id, 'albumName' => $albam->gallery_name]) }}" class="ssk ssk-twitter"></a>
            <a href="" data-url="{{ route('gallery',['id' => $albam->id, 'albumName' => $albam->gallery_name]) }}"  class="ssk ssk-facebook"></a>
            <a href="" data-url="{{ route('gallery',['id' => $albam->id, 'albumName' => $albam->gallery_name]) }}" class="ssk ssk-pinterest"></a>

            <div class = "shareLinkModal">
              <h3>Share Link</h3>
              <input type="text" value = "{{ route('gallery',['id' => $albam->id, 'albumName' => $albam->gallery_name]) }}">
            </div>
          </div>


       {{--<div class="album-details"> <img  style="object-fit:cover!important;" src="{{url('public/uploads/gallery/'.$albam->gallery_thumb)}}" alt="image" />--}}

        <div class="album-details"> <img  style="object-fit:cover!important;" src="{{url('public/uploads/gallery_coverphoto/'.$albam->cover_photo)}}" alt="image" />
     ...
     {{-- <div class="album-details"> <img  style="object-fit:cover!important;" src="{{url('public/uploads/gallery/'.$album->gallery_thumb)}}" alt="image" />
        <div class="description">
          <span class="album_name"> {{$albam->gallery_name}}</span><br>
          

          <div id="myDIV" >
            <p class="album_description">{!! substr($albam->description, 0, 200) !!}<span id="dots">...</span><span id="more">{{$albam->description}} </span> <span  id="toggle">Read more</span></p>
        ...
        @foreach($galleryImagesByname as $value)
                <div class="vf-box" id="vf-box{{$value->id}}"  onmouseenter="mouseEnter('{{$value->id}}')" onmouseleave="mouseLeave('{{$value->id}}')">
                    <a  class="fancybox-button" rel="fancybox-button" @if($image == 1) href="{{url('public/uploads/gallery/'.$value->image_thumb)}}" @endif>
                        <img src="{{url('public/uploads/gallery/thumb/'.$value->image_thumb)}}" alt="video" />
                    </a>
...
<script type="text/javascript">
    $(document).ready(function(){
        length = 300;
        cHtml = $(".content").html();
        cText = $(".content").html().substr(0, length);
        ...
        window.handler = function()
        {
            $('.exp').click(function(){
                if ($(".content").hasClass("compressed"))
                {
                    ...
                    myDIV.style.backgroundColor = "rgba(0, 0, 0, .4)";
                    handler();
                    return false;
                }
                else
